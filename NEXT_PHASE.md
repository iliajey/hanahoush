...
calls in a real browser, editorial workflow filter/detail-envelope defects, responsive
overflow at 1024px, broken media heading text, outline-button and marquee-text contrast, and
missing accessible names on case-study related-article links. The full browser matrix is green
against both the dev server and the production build.

- **Verification harness** — `frontend/playwright.config.ts` + `frontend/e2e/` (helpers +
  11 spec files) + `@axe-core/playwright`; evidence lives in `e2e-artifacts/` and
  `test-results/`.
- **Fixes** — API path normalization, editorial viewset fixes (+ backend regression tests),
  navbar/breakpoint/search-compaction, dashboard `min-w-0`, button/marquee contrast, media
  i18n dedupe, case-study link names, `<main>` landmark cleanup.
- **Build/backend** — typecheck/lint/test (216)/build/storybook green; backend check/
  migrations/bootstrap/pytest (281) green; no database reset.
- **ERP safety** — `ERP_ENABLED=false`, `ERP_PROVIDER=null`, zero ERP network calls.
...
`docs/architecture/hanrp-odoo-compatibility.md`. The Phase 10 browser harness can then be
used to verify the ERP surfaces end to end in a real browser.

**Option B (if the site continues to lead):**
**Phase 11 — Content & operations depth (priority)**: unlock the currently intentional
"not added" management surfaces (services/pages/analytics/company/settings) as the backend
grows real write endpoints; defer to the backend ACL codename-enforcement hardening phase
(articles.*/projects.*/media.*/analytics.view); consider prerender/SSR for the SPA SEO
ceiling; and convert the spreadsheet-style workspace tables into richer editorial tooling
(per-field HTML editing, cover pickers, inline previews) on the existing APIs.
