...
/** /projects — portfolio listing composed by the Page Builder. */
export function ProjectsPage() {
  const { t } = useTranslation()
  const { language } = useLanguage()
  const page = usePage("projects")

  useScrollDepth("projects")
  useSeoMeta(seoInput(page.data?.seo), language, { title: "Hanahoush — Projects" })
  ...
  if (page.isError || !page.data) {
    return (
      <main className="p-8">
        <ErrorState title="Couldn't load the projects page" description={t("errors.unexpected")} onRetry={() => page.refetch()} />
      </main>
    )
  }
  return (
